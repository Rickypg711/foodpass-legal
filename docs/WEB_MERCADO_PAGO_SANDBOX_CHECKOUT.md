...
Customer web ordering at `/menu/[restaurantId]` can pay online via **Checkout Pro** using the restaurant’s OAuth token stored in Firestore (server-side only).
...
3. `POST /api/mercado-pago/create-preference` (Admin SDK reads `mercadoPagoAccessToken` — never sent to browser).
